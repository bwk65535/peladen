...
- Tetap ada prompt password ketika ssh
    ...
    Terakhir, jika SELinux dalam keadaan _Enforcing_ di OS, maka pastikan juga label selinux contextnya sudah sesuai untuk folder `.ssh` dan file `authorized_keys`. Untuk merestore context yang benar, kita bisa menggunakan command `restorecon`.
    ```
